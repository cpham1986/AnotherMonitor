...
public class ViewGraphic extends TextureView {
    ...
    private List<Boolean> coreTotalD;
    private List<Paint> coreColors;
    ...
    private List<List<Float>> coreTotals;
...
//		canvas.drawPath(graphicPath, linesGridPaint);
		...
		//its possible we've identified the cores but no readings taken yet (when app starts)
        int cores;
        if(mSR.getCoreTotal().size() > 0){
             cores = mSR.getCoreTotal().get(0).size();
        } else {
            cores=-1;
        }

        if(coreColors.size() == cores && coreTotalD.size() == cores){
			for (int i = 0; i < coreTotalD.size(); i++) {
				if (coreTotalD.get(i)) {
					drawLineFloat(getPointsForCore(i,mSR.getCoreTotal()), canvas, coreColors.get(i));
				}
			}
		}
...
//		canvas.drawText("20%", tempVar, yTop + graphicHeight*0.8f + yLegendSpace, textPaintLegendV);
    ...
    private List<Float> getPointsForCore(int coreNumber, List<List<Float>> coreData){
    	List<Float> line = new ArrayList<>();
    	for(int i = 0; i < coreData.size(); i++){
    		line.add(coreData.get(i).get(coreNumber));
		}
    	return line;
	}
...
//		graphicPath =  new Path();
    ...
    void setService(ServiceReader sr) {
        ...
        coreTotals = mSR.getCoreTotal();
    }


    void setParameters(boolean cpuTotalD, boolean cpuAMD,
                       boolean memUsedD, boolean memAvailableD, boolean memFreeD, boolean cachedD, boolean thresholdD, List<Boolean> coreTotalD, List<Paint> coreColors) {
